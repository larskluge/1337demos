...
class MigrateStuffToPaperclip < ActiveRecord::Migration
  include PaperclipMigrations

  def self.up
    add_paperclip_fields :stuffs, :stuff_file
    Stuff.reset_column_information

    Stuff.all.each do |stuff|
      populate_paperclip_from_attachment_fu(stuff, stuff, 'stuff_file')
    end
  end
